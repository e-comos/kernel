/**
 * E-comOS Kernel - A kernel for E-comOS
 * Copyright (C) 2025, 2026 Junhao Ding(@Saladin5101)
 *
...
uint64_t pml4[PML4_ENTRIES]
	__attribute__((section(".paging"), aligned(PAGE_SIZE)));
uint64_t pdpt[PDPT_ENTRIES]
	__attribute__((section(".paging"), aligned(PAGE_SIZE)));
uint64_t pd[PD_ENTRIES] __attribute__((section(".paging"), aligned(PAGE_SIZE)));
uint64_t pt[NUM_PTS][PT_ENTRIES]
	__attribute__((section(".paging"), aligned(PAGE_SIZE)));
...
static int heap_expand(size_t size) {
	...
	if (!new_pages) {
		...
		for (size_t i = 0; i < pages_needed; i++) {
			...
			if (mm_map_page(HEAP_START_VIRT + (heap_end - HEAP_START_VIRT) +
								i * PAGE_SIZE,
		...
		for (size_t i = 0; i < pages_needed; i++) {
			if (mm_map_page(HEAP_START_VIRT + (heap_end - HEAP_START_VIRT) +
				...
				/* Cleanup on failure */
				for (size_t j = 0; j < i; j++) {
					mm_unmap_page(HEAP_START_VIRT +
								  (heap_end - HEAP_START_VIRT) + j * PAGE_SIZE);
				}
...
static void heap_split_block(heap_block_t* block, size_t size) {
	...
	heap_block_t* new_block =
		(heap_block_t*)((uintptr_t)block + sizeof(heap_block_t) + size);
...
static void heap_merge_blocks(void) {
	...
	while (current && current->next) {
		uintptr_t current_end =
			(uintptr_t)current + sizeof(heap_block_t) + current->size;
...
memory_status mm_init(boot_params* boot_params) {
	...
	if (!boot_params || !boot_params->memory_map ||
		boot_params->memory_map_size == 0 ||
		boot_params->memory_map_descriptor_size <
		...
		if (kern_start >= PHYS_BASE && kern_end > kern_start) {
			...
			uint32_t k_last =
				(uint32_t)((kern_end - PHYS_BASE + PAGE_SIZE - 1u) / PAGE_SIZE);
			if (k_last > MAX_PAGES)
				k_last = MAX_PAGES;
...
find_first:
	...
	for (uint32_t i = 0; i < HEAP_INITIAL_PAGES; i++) {
		...
		/* Map the page into kernel heap space */
		if (mm_map_page(HEAP_START_VIRT + i * PAGE_SIZE, (uintptr_t)page,
						MM_FLAG_KERNEL_RW) != 0) {
			mm_free_page(page);
	...
	if (mm_map_page(BOOT_INFO_ADDR, (uintptr_t)boot_info_page, MM_FLAG_USER) !=
		0) {
...
int handle_page_fault(uint64_t fault_addr, uint64_t error_code) {
	...
	if (fault_addr >= 0x400000 && fault_addr < 0x800000) {
		...
		if (mm_map_page(page_start, (uintptr_t)page,
